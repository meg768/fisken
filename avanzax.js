...
var Module = module.exports = function() {
	...
	function getAccountsOverview(session) {
		...
		*/
		...
		return getJSON(session, sprintf('https://www.avanza.se/_mobile/account/overview'));
	}
	...
	function getAccounts(session) {
		...
		*/
		...
		return getJSON(session, sprintf('https://www.avanza.se/_mobile/account/list?onlyTradable=false'));
	}
	...
	function getJSON(session, url) {
		return new Promise(function(resolve, reject) {
			...
			options.method = 'GET';
			options.url    = url;
			options.headers['X-AuthenticationSession'] = session.authenticationSession;
	...
	function getStock(session, id) {
		return getJSON(session, sprintf('https://www.avanza.se/_mobile/market/stock/%s', id));
	}

	function buy(session, accountId, orderbookId, volume) {
		return new Promise(function(resolve, reject) {
	...
	function sell(session, accountId, orderbookId) {
		return new Promise(function(resolve, reject) {
	...
	function getPositions(session, accountId) {
		...
		*/
		...
		return getJSON(session, sprintf('https://www.avanza.se/_mobile/account/%s/positions', accountId));

	}

	function postJSON(session, url, payload) {
		return new Promise(function(resolve, reject) {
	...
	function run() {
		...
		login(config.credentials).then(function(session) {

			getAccounts(session).then(function(accounts) {
				...
				accounts.forEach(function(account) {
					ownCapital += account.ownCapital;

					console.log(sprintf('%s - %.0f', account.name, account.ownCapital));
				});

				console.log(sprintf('Totalt - %.0f', ownCapital));


				return Promise.resolve(accounts[2]);

			})

			.then(function(account) {
				return getPositions(session, account.id);
			})
			.then(function(positions) {
				return buy(session, '8404958', '199531', 2000);
//				return console.log(JSON.stringify(positions, null, '\t'));
			})
